        ...
        try:
            ...
            for cart_item in cart_items:
                if not Product.objects.filter(id=cart_item.product.id).exists():
                    return Response(
                        {'error': 'A product with ID provided does not exits'},
                        status=status.HTTP_404_NOT_FOUND
                    )
                if int(cart_item.count) > int(cart_item.product.quantity):
                    return Response(
                        {'error': 'Not enought items in stock'},
                        status=status.HTTP_200_OK
                    )
...
class ProcessPaymentView(APIView):
    def post(self, request, format=None):
        ...
        # Revisar si usuario tiene items en carrit
        if not CartItem.objects.filter(cart=cart).exists():
            return Response(
                {'error': 'Need to have items in cart'},
                status=status.HTTP_404_NOT_FOUND
        ...
        for cart_item in cart_items:
            if not Product.objects.filter(id=cart_item.product.id).exists():
                return Response(
                    {'error': 'Transaction failed, a product ID does not exits'},
                    status=status.HTTP_404_NOT_FOUND
                )
            if int(cart_item.count) > int(cart_item.product.quantity):
                return Response(
        ...
        try:
            # Crear transacción con braintree
        ...
        print(newTransaction.is_success)
        ...
        if newTransaction.is_success or newTransaction.transaction:
            ...
            for cart_item in cart_items:
                try:
                    ...
                    return Response(
                        {'error': 'Transaction succeded and order created, but failed'},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR
                    )
            ...
            try:
                ...
                return Response(
                    {'error': 'Transaction succeded and order created, but failed to send'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
            ...
            try:
                ...
                return Response(
                    {'error': 'Transaction succeded and order successful, but failed to '},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
