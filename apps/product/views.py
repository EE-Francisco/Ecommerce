...
class ListSearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        # Check input
        if len(search) == 0:
            # Show all products if there is not input
            search_results = Product.objects.order_by('-date_created').all()
        else:
            # If there was search criteria, filter by description as name containing search data
        ...
        if not Category.objects.filter(id=category_id).exists():
            return Response(
                {'error': 'Category noy found'},
                status=status.HTTP_404_NOT_FOUND
            )

        category = Category.objects.get(id=category_id)

        # If the category has a parent, filter only by category
        if category.parent:
            search_results = search_results.order_by(
                '-date_created'
            ).filter(category=category)
        else:
            # If it have not children, filter only by category
...
class ListBySearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        # If categoryId is 0, fiilter all categories
        if category_id == 0:
